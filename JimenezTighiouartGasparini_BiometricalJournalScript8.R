...
#File: JimenezTighiouartGasparini_BiometricalJournalScript8.R
...
attribError = 0.1 #Probability of missattributing a DLT
...
for (kk in 1:M) {
  ...
  #We are going to incorrecly attribute the toxicity in for 10% or 50% of the attributions.
  
  if((resp1[[1]] | resp2[[1]])==1 & (resp1[[2]] | resp2[[2]])==1 & runif(1)<=attribError){
    
    #We missattribute the DLT for patient 1
    
    if(resp1[[2]] == 1 & resp1[[3]] == 1 & resp1[[4]] == 0){
      resp1[[3]] = 0
      resp1[[4]] = 1
    }else if(resp1[[2]] == 1 & resp1[[3]] == 0 & resp1[[4]] == 1){
      resp1[[3]] = 1
      resp1[[4]] = 0
    }else if(resp1[[2]] == 1 & resp1[[3]] == 1 & resp1[[4]] == 1){
      resp1[[2]] = 0
      resp1[[3]] = 0
      resp1[[4]] = 0
    }
    
    #We missattribute the DLT for patient 2
    
    if(resp2[[2]] == 1 & resp2[[3]] == 1 & resp2[[4]] == 0){
      resp2[[3]] = 0
      resp2[[4]] = 1
    }else if(resp2[[2]] == 1 & resp2[[3]] == 0 & resp2[[4]] == 1){
      resp2[[3]] = 1
      resp2[[4]] = 0
    }else if(resp2[[2]] == 1 & resp2[[3]] == 1 & resp2[[4]] == 1){
      resp2[[2]] = 0
      resp2[[3]] = 0
      resp2[[4]] = 0
    }
  ...
  #We are going to incorrecly attribute the toxicity in 10% or 50% of the attributions.
...
true = true1[c(2,3,4,5,6,7,9),] #These rows correspond to the MTD set.
colnames(true) = c("x","y","prob","id")


#Code to obtain Tables S4 and S5 in the supplementary material
...
for (i in 1:M){
  ...
  num.true.doses.recommended = nrow(merge(result[result$trial==i,],true,by=c("x","y")))
  
  if(num.doses.recommended == 0 & num.true.doses.recommended == 0){
    ...
    if((num.true.doses.recommended/num.doses.recommended)>0.5){
      atleast50[i]=1
    }
